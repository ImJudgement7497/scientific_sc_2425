...
```txt
 ...
 45.18    708.74   333.13 14183404272     0.00     0.00  
 std::vector<double, std::allocator<double> >::operator[](unsigned long)
  3.71    736.11    27.38 19135071     0.00     0.00  
...
This was the major change of the code. There were no race conditions to update the vectors (`force_*`) as each `i` is private to the thread, and each thread will have a different `i` concurrently, thus no two threads are accessing the same part of memory at once. 
...
Running on `GViking` with scripts in `run_scripts` and `job_scripts`, I ran a scaling test on the modified code from 1 -> 96 threads. The table below shows the actual time taken and the speedup for each thread run compared to 1 thread. These results can be found in the `data` directory, and also the logs in `logs`. After each run, I compared the output files to `first_run` using `diff` and no differences were seen.
