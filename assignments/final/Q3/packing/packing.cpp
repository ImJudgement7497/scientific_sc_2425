...
void calculate_forces(std::vector<double>& x,
   ...
   for (int i = 0; i< np; i++){
      ...
      // in the plane
      if(bmx) pbc_loop(x, y, z, particle_radius, fx, fy, fz, -Lx, 0.0, 0.0, force_size, rix, riy, riz, rir); //-x,0,0
      if(bpx) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  Lx, 0.0, 0.0, force_size, rix, riy, riz, rir); //+x,0,0
      if(bpy) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0,  Ly, 0.0, force_size, rix, riy, riz, rir); //+y,0,0
      if(bmy) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0, -Ly, 0.0, force_size, rix, riy, riz, rir); //-y,0,0
      if(bpx && bpy) pbc_loop(x, y, z, particle_radius, fx, fy, fz,   Lx, Ly, 0.0, force_size, rix, riy, riz, rir); //+x,+y,0
      if(bpx && bmy) pbc_loop(x, y, z, particle_radius, fx, fy, fz,   Lx, -Ly, 0.0, force_size, rix, riy, riz, rir); //+x,-y,0
      if(bmx && bpy) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  -Lx, Ly, 0.0, force_size, rix, riy, riz, rir); //-x,+y,0
      if(bmx && bmy) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  -Lx, -Ly, 0.0, force_size, rix, riy, riz, rir); //-x,-y,0

      // up
      if(bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0, 0.0, Lz, force_size, rix, riy, riz, rir); //0,0,+z
      if(bmx && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, -Lx, 0.0, Lz, force_size, rix, riy, riz, rir); //-x,0,+z
      if(bpx && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  Lx, 0.0, Lz, force_size, rix, riy, riz, rir); //+x,0,+z
      if(bpy && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0,  Ly, Lz, force_size, rix, riy, riz, rir); //+y,0,+z
      if(bmy && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0, -Ly, Lz, force_size, rix, riy, riz, rir); //-y,0,+z
      if(bpx && bpy && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  Lx, Ly, Lz, force_size, rix, riy, riz, rir); //+x,+y,+z
      if(bpx && bmy && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,   Lx, -Ly, Lz, force_size, rix, riy, riz, rir); //+x,-y,+z
      if(bmx && bpy && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  -Lx, Ly, Lz, force_size, rix, riy, riz, rir); //-x,+y,+z
      if(bmx && bmy && bpz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  -Lx, -Ly, Lz, force_size, rix, riy, riz, rir); //-x,-y,+z

      // down
      if(bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0, 0.0, -Lz, force_size, rix, riy, riz, rir); //0,0,-z
      if(bmx && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, -Lx, 0.0, -Lz, force_size, rix, riy, riz, rir); //-x,0,-z
      if(bpx && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  Lx, 0.0, -Lz, force_size, rix, riy, riz, rir); //+x,0,-z
      if(bpy && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0,  Ly, -Lz, force_size, rix, riy, riz, rir); //+y,0,-z
      if(bmy && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz, 0.0, -Ly, -Lz, force_size, rix, riy, riz, rir); //-y,0,-z
      if(bpx && bpy && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  Lx, Ly, -Lz, force_size, rix, riy, riz, rir); //+x,+y,-z
      if(bpx && bmy && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,   Lx, -Ly, -Lz, force_size, rix, riy, riz, rir); //+x,-y,-z
      if(bmx && bpy && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  -Lx, Ly, -Lz, force_size, rix, riy, riz, rir); //-x,+y,-z
      if(bmx && bmy && bmz) pbc_loop(x, y, z, particle_radius, fx, fy, fz,  -Lx, -Ly, -Lz, force_size, rix, riy, riz, rir); //-x,-y,-z
...
int main(){
   ...
   for (int i=0; i<num_particles; i++){
      ...
      if(bin_ID<200) volume_bins[bin_ID]+=1.0;

      double r = particle_radius[i];
      int rbin_ID = int((r+0.5)/1.0);
      if(rbin_ID<200) radius_bins[rbin_ID]+=1.0;
   ...
   for(int i = 0; i< particle_volume.size(); i++) vol_spheres += particle_volume[i];
