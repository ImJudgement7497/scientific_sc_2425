...
The `check_overlap()` function is effectivly the same as before, but now threads call it in parallel to check. This only reads the grid, so is safe to do in parallel. If there is an overlap found, the flag on the circle is set. As each thread will have it's own unique trial placement to set the flag on, this is also thread safe. 

After a synchronisation, there is an internal check beyween trials, in the form of `O(N^2)` loop. To avoid the cost of this however, we skip any set flags. Each thread has it's own unique sequences to check within the trials. The implementation of similar grid logic as the serial version was tried here, however it was very difficult to make it thread safe and caused more slow down, as all the trials needed to be placed in another grid first. The `O(N^2)` loop does gurantee that no circles can overlap.

There is an issue that lays here however, where different runs of the code with the same number of threads will likely output different results of the number of circles. This is due to the a couple of factors, but mostly the differently seeded random number generators. If the code implemented an exact ordering for when the threads execute, I suspect this problem will likely go away. However at that point, the code is relativly serial. To actually achieve decent parallel speedup, it cannot be exactly determined when the threads will execute certain functions, thus different orders of execution will achieve different results. The main work into this problem was to reduce these differences, by implementing thread safe approached such as the flag approach and read only. This way, the number of circles placed is within the same order for each run.

Due to this, I wrote an averaging function, to be ran on `VIKING` by binary `main_parallel_viking`, that executed the simulation five times. This then gave a mean number of circles and mean execution time. This helped with the more "random" behaviour.
